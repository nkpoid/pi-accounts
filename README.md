...
標準の Codex またはこの拡張のアカウントへの OAuth ログインが成功すると、そのアカウントを新規セッションのデフォルトに保存する。
Pi 標準の `settings.json` の `defaultProvider` と `defaultModel` を使い、次回起動や `/new` で読み込む。
モデルは、既存のデフォルト、現在のモデル、ログイン先のカタログの先頭の順に、利用可能なものを選ぶ。
ログインのキャンセルや認証保存の失敗では、デフォルトを更新しない。

**実行中のセッションの送信先は変更しない。**
ほかのセッションへのリアルタイムな追従も行わず、再開したセッションでは保存済みの選択を復元する。
...
`/account` 自体は新規セッションのデフォルトを変更せず、ログイン成功時に更新する。
...
Pi 0.85.1 の TUI でも、合成認証情報で現在の会話を変えずに保存し、再起動と `/new` 後に保存したアカウントとフッター表示を復元することを確認した。

Pi 0.85.1 にはログイン完了イベントがないため、デフォルト保存はセッション固有の `ModelRuntime.login()` 完了をフックしている。
